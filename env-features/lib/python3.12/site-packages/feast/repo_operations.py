...
def _get_repo_contents(
    ...
    if len(repo.projects) < 1:
        if project_name:
            print(
                f"No project found in the repository. Using project name {project_name} defined in features.yaml"
            )
            project_description = (
            ...
            print(
                "No project found in the repository. Either define Project in repository or define a project in features.yaml"
            )
            sys.exit(1)
    elif len(repo.projects) == 1:
        if repo.projects[0].name != project_name:
            print(
                "Project object name should match with the project name defined in features.yaml"
            )
            sys.exit(1)
    else:
        print(
...
def create_feature_store(
    ...
    repo = ctx.obj["CHDIR"]
    # If we received a base64 encoded version of features.yaml, use that
    config_base64 = os.getenv(FEATURE_STORE_YAML_ENV_NAME)
    if config_base64:
        print("Received base64 encoded features.yaml")
        config_bytes = base64.b64decode(config_base64)
        # Create a new unique directory for writing features.yaml
        repo_path = Path(tempfile.mkdtemp())
        with open(repo_path / "features.yaml", "wb") as f:
...
def init_repo(repo_name: str, template: str, repo_path: Optional[str] = None):
    ...
    repo_config_path = target_path / "features.yaml"
    ...
    # Template the features.yaml file
    feature_store_yaml_path = target_path / "feature_repo" / "features.yaml"
    replace_str_in_file(
